                            ...
                            if ($order['OrderStatus'] == 'Completed') {
                                ...
                                if ($allowInsert) {
                                    if ($conn->query($query)) {
                                        ...
                                        logMessage("Iertnsion failed for OrderID {$OrderID}: " . $conn->error, 'error');
                                    }
                                }
                            }
                        } else {
...
try {
    ...
    $siteID = 0;
    $verb = 'GetOrders';
    $CreateTimeFrom = gmdate("Y-m-d\TH:i:s", time() - 3600 * 36); 
    ...
    $accountRow = [
            "account_username" => "impalaa_online",
            "id" => "445",
            "price_tag" => "1"
        ];
    
    // while ($accountRow = $getAccounts->fetch_assoc()) {
    //  echo '$accountRow: <pre>' .print_r($accountRow,true). '</pre>'; 
        $userToken = "v^1.1#i^1#f^0#I^3#r^1#p^3#t^Ul4xMF8wOjcxN0MzQzI2MEU5NjVBNERBMEY1NjY3RjdBQUQ5NUUzXzFfMSNFXjI2MA==";
        $AccountID = "445";
        ...
        try {
            ...
            logMessage("ðŸ”’ Token marked invalid for Account ID: {$AccountID}");
        }
