                ...
                foreach($orders['payload']['Orders'] as $order){
                    ...
                    $check_order = $conn->query("select * from app_orders where OrderID = '$OrderID' && OrderType='4'");
                     if($check_order->num_rows == 0){
                        ...
                        if(($order['OrderStatus'] == 'Unshipped' || $order['OrderStatus'] == 'PartiallyShipped' || $order['OrderStatus'] == 'Shipped')){
...
$getAccounts = $conn->query("select * from app_accounts where id = 374 &&  active = '1' && auth_token != '' && account_type = '4' order by id desc");
