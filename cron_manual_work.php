...
function getImportOrdersEbay($conn, $accountRow, $siteID, $verb, $CreateTimeFrom, $CreateTimeTo, $userToken, $AccountID, $devID, $appID, $certID, $serverUrl, $compatabilityLevel, $page)
{
    ...
    if (stristr($responseXml, 'HTTP 404') || $responseXml == '') {
        ...
        if ($res['Ack'] == 'Success' || $res['Ack'] == 'Warning') {
                ...
                foreach ($res['OrderArray']['Order'] as $order) {
                    ...
                    if ($check_order->num_rows == 0) {
                        ...
                        if ($order['OrderStatus'] == 'Completed') {
                            ...
                            if ($allowInsert) {
                                if (!$conn->query($query)) {
                                    logMessage("Insertion failed for OrderID {$OrderID}: " . $conn->error, 'error');
                                }
                            }
                        }
                    }
            ...
            if (in_array($res['Errors']['ErrorCode'], [931, 17470, 841, 21916013])) {
...
try {
    ...
    // Date range from 23 September 2025 to 1 October 2025
    $CreateTimeFrom = '2025-09-23T00:00:00';
    $CreateTimeTo   = '2025-10-01T23:59:59';

    $accountID = 503; // Only fetch for this account
    $getAccounts = $conn->query("SELECT * FROM app_accounts WHERE id = '$accountID' LIMIT 1");
